  ...
  function baseCss(config) {
    ...
    return `
      ...
      .media-task { position:absolute; right:8px; bottom:8px; max-width:70%; padding:4px 7px; border-radius:999px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; background:rgba(0,0,0,.72); font-size:11px; }
        .media-ha-picture-camera {
          background: #000;
        }
        .media-ha-picture-camera printer-control-center-inline-camera {
          display: block;
          width: 100%;
          height: 100%;
          min-height: inherit;
        ...
        .media-stable-ha-camera {
          background: #000;
        }
        .media-stable-ha-camera printer-control-center-inline-camera {
          display: block;
          width: 100%;
          height: 100%;
          min-height: inherit;
...
<main><img id="cam" src="${sourceHtml}" alt="${title}"></main>
  ...
  class BaseCard extends HTMLElement {
    ...
    bindInlineCameras() {
      ...
      this.shadowRoot.querySelectorAll("printer-control-center-inline-camera").forEach((node) => {
  ...
  class CompleteCard extends BaseCard {
    ...
    render() {
      ...
      if(size==="s"){
        ...
        `);
        this.bind(map); this.bindInlineCameras(); return;
      }
      ...
      this.shadowRoot.innerHTML=frame(this._config,`
        ...
        <div class="footer"><span>3D-Printer Control Center ${VERSION}</span><a href="${DOCS}" target="_blank">${DOCS}</a></div>
      `);
      this.bind(map); this.bindInlineCameras();
    }
  }
...
;(() => {
  const CARD_TYPE = "printer-control-center-inline-camera";
  if (customElements.get(CARD_TYPE)) return;

  const GLOBAL_KEY = "__pccV419InlineCameraRegistry";
  const registry = window[GLOBAL_KEY] || {
    cards: new Map(),
    loading: new Map(),
    parking: null,
    helperPromise: null,
  ...
  function ensureParking() {
    if (registry.parking && document.body && document.body.contains(registry.parking)) {
      return registry.parking;
    }
    const parking = document.createElement("div");
    parking.setAttribute("data-pcc-v419-camera-parking", "1");
    ...
    if (document.body) document.body.appendChild(parking);
    registry.parking = parking;
    return parking;
  }

  async function createHaCameraCard(entity) {
    const config = {
      type: "picture-entity",
      entity,
      camera_image: entity,
    ...
    if (window.loadCardHelpers) {
      registry.helperPromise = registry.helperPromise || window.loadCardHelpers();
      const helpers = await registry.helperPromise;
  ...
  class PrinterControlCenterInlineCamera extends HTMLElement {
    constructor() {
      super();
      this._hass = null;
      this._entity = "";
    ...
    disconnectedCallback() {
      if (this._card) {
        ...
        parking.appendChild(this._card);
      }
    }
    ...
    _installShell() {
      if (!this.shadowRoot || this.shadowRoot.querySelector("#host")) return;
      this.shadowRoot.innerHTML = `
        <style>
          :host {
            display: block;
    ...
    async _render() {
      ...
      if (this._card && this._entity === entity) {
        if (this._card.parentElement !== host) host.appendChild(this._card);
        if (this._hass) this._card.hass = this._hass;
        return;
      }

      if (registry.cards.has(entity)) {
        this._card = registry.cards.get(entity);
        this._entity = entity;
        host.innerHTML = "";
      ...
      if (registry.loading.has(entity)) {
        const card = await registry.loading.get(entity);
        this._card = card;
        this._entity = entity;
        host.innerHTML = "";
      ...
      const loading = createHaCameraCard(entity)
        .then((card) => {
          registry.cards.set(entity, card);
          registry.loading.delete(entity);
          return card;
        })
        .catch((err) => {
          registry.loading.delete(entity);
      ...
      registry.loading.set(entity, loading);

      try {
        const card = await loading;
        this._card = card;
        this._entity = entity;
        host.innerHTML = "";
        host.appendChild(card);
        ...
        console.error("PCC stable inline HA camera failed", err);
        host.innerHTML = `<div class="empty">Livekamera konnte nicht geladen werden.</div>`;
      }
    }
  }

  customElements.define(CARD_TYPE, PrinterControlCenterInlineCamera);
})();
