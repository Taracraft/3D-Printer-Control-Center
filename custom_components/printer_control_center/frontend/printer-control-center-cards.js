/* 3D-Printer Control Center - HACS Release 5.0.0-beta37*/
(() => {
  const VERSION = "5.0.0-beta37";
...
(() => {
  const STUDIO_VERSION = "5.0.0-beta37";
...
const PCC_BETA37_STUDIO_CLASS = customElements.get("printer-control-center-studio-card") || PrinterControlCenterStudioCard;
  const PCC_BETA37_PREV_CLEANUP = PCC_BETA37_STUDIO_CLASS.prototype.cleanupBetaStudioUi;
  const PCC_BETA37_PREV_RENDER = PCC_BETA37_STUDIO_CLASS.prototype.render;
  const PCC_BETA37_PREV_APPLY_ACTIVE_JOB = PCC_BETA37_STUDIO_CLASS.prototype.applyActiveJob;
  const PCC_BETA37_PREV_BETA7_IMPORT_SELECTED = PCC_BETA37_STUDIO_CLASS.prototype.beta7ImportSelected;
  const PCC_BETA37_PREV_BETA7_IMPORT_PLAN = PCC_BETA37_STUDIO_CLASS.prototype.beta7ImportPlan;
  const PCC_BETA37_PREV_RENDER_MESH = PCC_BETA37_STUDIO_CLASS.prototype.renderMeshCanvas;

  function pccBeta37Number(value, fallback=0) {
    const n = Number(value);
    return Number.isFinite(n) ? n : fallback;
  }

  function pccBeta37Transform(instance) {
    const base = typeof defaultTransform === "function"
      ? defaultTransform()
      : {x:0,y:0,z:0,rx:0,ry:0,rz:0,scale:100,sx:100,sy:100,sz:100,skewX:0,skewY:0};
    instance._transform = {...base, ...(instance._transform || {}), ...(instance._activeJob?.transform || {})};
  ...
  function pccBeta37JobId(job) {
    return String(job?.id || job?.file_path || job?.path || job?.name || job?.modelName || job?.model?.path || job?.model?.name || "").trim();
  }

  function pccBeta37Preview(job) {
    const values = [
      job?.preview_data_url,
      job?.preview_url,
      job?.thumbnail,
    ...
    return values.map((value) => String(value || "").trim()).find((value) => value && value !== "[object Object]") || "";
  }

  function pccBeta37HasActiveVisual(instance) {
    return Boolean(instance?._activeJob || instance?._studioModelImageUrl || instance?._pccBeta37LastPreview || instance?._pccBeta36PlateObjects?.length || instance?._pccBeta35PlateObjects?.length);
  }

  PCC_BETA37_STUDIO_CLASS.prototype.beta37EnsureStyle = function beta37EnsureStyle() {
    const root = this.shadowRoot;
    if (!root || root.querySelector("#pcc-beta37-stable-import-object-style")) return;

    const style = document.createElement("style");
    style.id = "pcc-beta37-stable-import-object-style";
    style.textContent = `
      .pcc-beta37-no-geometry-hidden,
      .pcc-beta37-context-hidden,
      .studio-context,
      .beta4-floating-context,
      .pcc-context-menu,
      .context-menu,
      ...
      .buildplate.pcc-beta37-dragging{
        cursor:grabbing!important;
      }

      .pcc-beta36-imported-image,
      .pcc-beta37-imported-image{
        position:absolute!important;
        left:50%!important;
        top:50%!important;
        width:min(34%,260px)!important;
      ...
      .pcc-beta36-imported-placeholder,
      .pcc-beta37-imported-placeholder{
        position:absolute!important;
        left:50%!important;
        top:50%!important;
        width:180px!important;
  ...
  PCC_BETA37_STUDIO_CLASS.prototype.beta37HideNoGeometryOverlay = function beta37HideNoGeometryOverlay() {
    const root = this.shadowRoot;
    if (!root) return;

    const active = pccBeta37HasActiveVisual(this);
    if (!active) return;

    root.querySelectorAll("div,span,small,p").forEach((node) => {
      if (node.closest(".pcc-beta27-import-modal")) return;
      if (node.closest(".pcc-beta9-plate-selector")) return;
      if (node.closest(".pcc-beta30-toolbar")) return;
      ...
      const text = String(node.textContent || "").trim();
      if (
        text === "Keine aktive Geometrie geladen" ||
        text.includes("Keine aktive Geometrie geladen") ||
        text.includes("Kein Modell geladen") ||
        text.includes("kein Modell geladen")
      ) {
        node.classList.add("pcc-beta37-no-geometry-hidden");
        node.remove();
      }
    });
  };

  PCC_BETA37_STUDIO_CLASS.prototype.beta37RemoveContextMenus = function beta37RemoveContextMenus() {
    const root = this.shadowRoot;
    if (!root) return;
    this._studioContextMenu = null;
    root.querySelectorAll(".studio-context,.beta4-floating-context,.pcc-context-menu,.context-menu,.gallery-context-menu,[data-context-menu]").forEach((node) => {
      node.classList.add("pcc-beta37-context-hidden");
      node.remove();
    });
  };

  PCC_BETA37_STUDIO_CLASS.prototype.beta37ApplyVisualTransform = function beta37ApplyVisualTransform() {
    const root = this.shadowRoot;
    const t = pccBeta37Transform(this);
    const transform = `translate(-50%,-50%) translate(${pccBeta37Number(t.x)}px,${pccBeta37Number(t.y)}px) rotateX(${pccBeta37Number(t.rx)}deg) rotateZ(${pccBeta37Number(t.rz)}deg) scale(${pccBeta37Number(t.scale,100)/100})`;
    root?.querySelectorAll(".pcc-beta36-imported-image,.pcc-beta36-imported-placeholder,.pcc-beta37-imported-image,.pcc-beta37-imported-placeholder").forEach((node) => {
      node.style.transform = transform;
    });
  };

  PCC_BETA37_STUDIO_CLASS.prototype.beta37PlaceActiveVisual = function beta37PlaceActiveVisual(force=false) {
    const root = this.shadowRoot;
    const buildplate = root?.querySelector(".buildplate");
    if (!root || !buildplate) return;

    const job = this._activeJob;
    const preview = pccBeta37Preview(job) || this._studioModelImageUrl || this._pccBeta37LastPreview || "";
    const label = String(job?.name || job?.modelName || job?.file_name || job?.model?.name || "Objekt").trim();

    if (!job && !preview && !force) return;

    if (preview) {
      this._pccBeta37LastPreview = preview;
      this._studioModelImageUrl = this._studioModelImageUrl || preview;
    }

    let node = root.querySelector(".pcc-beta37-imported-image,.pcc-beta36-imported-image,.pcc-beta37-imported-placeholder,.pcc-beta36-imported-placeholder");

    if (!node) {
      if (preview) {
        node = document.createElement("img");
        node.className = "pcc-beta37-imported-image";
        node.src = preview;
        node.alt = label;
      } else {
        node = document.createElement("div");
        node.className = "pcc-beta37-imported-placeholder";
        node.textContent = label;
      }
      buildplate.appendChild(node);
    } else {
      if (preview && node.tagName !== "IMG") {
        node.remove();
        node = document.createElement("img");
        node.className = "pcc-beta37-imported-image";
        node.src = preview;
        node.alt = label;
        buildplate.appendChild(node);
      } else if (preview && node.tagName === "IMG" && node.getAttribute("src") !== preview) {
        node.setAttribute("src", preview);
        node.setAttribute("alt", label);
      } else if (!preview && node.tagName !== "IMG") {
        node.textContent = label;
      }
    }

    this.beta37ApplyVisualTransform();
    this.beta37HideNoGeometryOverlay();
    this.beta37BindDragSurface();
  };

  PCC_BETA37_STUDIO_CLASS.prototype.beta37StabilizeActiveJob = function beta37StabilizeActiveJob(job) {
    if (!job) return;

    const id = pccBeta37JobId(job);
    if (id) {
      job.id = job.id || id;
      this._activeJobId = id;
    }

    job.transform = {...pccBeta37Transform(this), ...(job.transform || {})};
    this._activeJob = job;

    const preview = pccBeta37Preview(job);
    if (preview) {
      this._pccBeta37LastPreview = preview;
      this._studioModelImageUrl = preview;
    }
    ...
    if (Array.isArray(this._jobs)) {
      const key = pccBeta37JobId(job);
      const idx = this._jobs.findIndex((item) => pccBeta37JobId(item) === key);
      if (idx >= 0) this._jobs[idx] = {...this._jobs[idx], ...job};
      else this._jobs.push(job);
    } else {
      this._jobs = [job];
    ...
    this.beta37PlaceActiveVisual(true);
  };

  PCC_BETA37_STUDIO_CLASS.prototype.applyActiveJob = function beta37ApplyActiveJob(job, options={}) {
    if (typeof PCC_BETA37_PREV_APPLY_ACTIVE_JOB === "function") {
      PCC_BETA37_PREV_APPLY_ACTIVE_JOB.call(this, job, options);
    }

    this.beta37StabilizeActiveJob(this._activeJob || job);

    window.setTimeout(() => this.beta37PlaceActiveVisual(true), 40);
    window.setTimeout(() => this.beta37PlaceActiveVisual(true), 180);
  };

  PCC_BETA37_STUDIO_CLASS.prototype.beta7ImportSelected = async function beta37ImportSelected(...args) {
    if (typeof PCC_BETA37_PREV_BETA7_IMPORT_SELECTED === "function") {
      await PCC_BETA37_PREV_BETA7_IMPORT_SELECTED.apply(this, args);
    }

    this.beta37StabilizeActiveJob(this._activeJob);

    window.setTimeout(() => this.beta37PlaceActiveVisual(true), 40);
    window.setTimeout(() => this.beta37PlaceActiveVisual(true), 180);
    window.setTimeout(() => this.beta37PlaceActiveVisual(true), 420);
  };

  PCC_BETA37_STUDIO_CLASS.prototype.beta7ImportPlan = async function beta37ImportPlan(plan) {
    if (typeof PCC_BETA37_PREV_BETA7_IMPORT_PLAN === "function") {
      await PCC_BETA37_PREV_BETA7_IMPORT_PLAN.call(this, plan);
    }

    this.beta37StabilizeActiveJob(this._activeJob || plan);
  ...
  PCC_BETA37_STUDIO_CLASS.prototype.renderMeshCanvas = function beta37RenderMeshCanvas(...args) {
    if (typeof PCC_BETA37_PREV_RENDER_MESH === "function") {
      PCC_BETA37_PREV_RENDER_MESH.apply(this, args);
    }

    this.beta37PlaceActiveVisual(false);
  };

  PCC_BETA37_STUDIO_CLASS.prototype.render = function beta37Render(...args) {
    let result;
    if (typeof PCC_BETA37_PREV_RENDER === "function") {
      result = PCC_BETA37_PREV_RENDER.apply(this, args);
    }

    this.beta37FinalCleanup();

    requestAnimationFrame(() => this.beta37FinalCleanup());
    window.setTimeout(() => this.beta37FinalCleanup(), 80);

    return result;
  };

  PCC_BETA37_STUDIO_CLASS.prototype.beta37BindDragSurface = function beta37BindDragSurface() {
    const root = this.shadowRoot;
    const buildplate = root?.querySelector(".buildplate");
    if (!root || !buildplate || buildplate.dataset.beta37DragBound === "1") return;

    buildplate.dataset.beta37DragBound = "1";

    const isInsidePlate = (event) => {
      const path = event.composedPath?.() || [];
      return path.includes(buildplate) || Boolean(event.target?.closest?.(".buildplate,.pcc-beta36-imported-image,.pcc-beta36-imported-placeholder,.pcc-beta37-imported-image,.pcc-beta37-imported-placeholder"));
    };

    const removeMenus = () => this.beta37RemoveContextMenus();

    const blockContext = (event) => {
      if (!isInsidePlate(event)) return;
      event.preventDefault();
      event.stopPropagation();
      removeMenus();
    };
    ...
    buildplate.addEventListener("contextmenu", blockContext, {capture:true});
    window.addEventListener("contextmenu", blockContext, true);
    root.addEventListener("auxclick", blockContext, {capture:true});
    buildplate.addEventListener("auxclick", blockContext, {capture:true});
    ...
    const moveHandler = (event) => {
      ...
      if (state.button === 0) {
        t.rz = pccBeta37Number(t.rz) + dx * 0.32;
        t.rx = pccBeta37Number(t.rx) - dy * 0.22;
      }

      if (state.button === 2) {
        t.x = pccBeta37Number(t.x) + dx;
        t.y = pccBeta37Number(t.y) + dy;
      }
    ...
    const upHandler = (event) => {
      ...
      state.active = false;
      state.button = -1;
      buildplate.classList.remove("pcc-beta37-dragging");
    ...
    const downHandler = (event) => {
      if (!isInsidePlate(event)) return;
      if (event.target?.closest?.(".pcc-beta27-import-modal,.pcc-beta9-plate-selector")) return;
      if (event.button !== 0 && event.button !== 2) return;
      ...
      buildplate.classList.add("pcc-beta37-dragging");
      removeMenus();
    ...
    root.addEventListener("pointerdown", downHandler, {capture:true});
    buildplate.addEventListener("pointerdown", downHandler, {capture:true});
    root.addEventListener("mousedown", downHandler, {capture:true});
    buildplate.addEventListener("mousedown", downHandler, {capture:true});
  };

  PCC_BETA37_STUDIO_CLASS.prototype.handleContextMenu = function beta37HandleContextMenu(event) {
    if (event.target?.closest?.(".buildplate,.pcc-beta36-imported-image,.pcc-beta36-imported-placeholder,.pcc-beta37-imported-image,.pcc-beta37-imported-placeholder")) {
      event.preventDefault();
      event.stopPropagation();
      this.beta37RemoveContextMenus();
    }
  };

  PCC_BETA37_STUDIO_CLASS.prototype.beta37FinalCleanup = function beta37FinalCleanup() {
    this.beta37EnsureStyle();

    try { this.beta34CleanTextNavigation?.(); } catch (_error) {}
    try { this.beta34RemoveFooters?.(); } catch (_error) {}
    try { this.beta36HideSecondImportButton?.(); } catch (_error) {}
    try { this.beta36BindToolbar?.(); } catch (_error) {}
    try { this.beta35PolishImportPopup?.(); } catch (_error) {}
  ...
  PCC_BETA37_STUDIO_CLASS.prototype.cleanupBetaStudioUi = function beta37CleanupBetaStudioUi(...args) {
    if (typeof PCC_BETA37_PREV_CLEANUP === "function") {
      try { PCC_BETA37_PREV_CLEANUP.apply(this, args); } catch (_error) {}
    }

    this.beta37FinalCleanup();

    requestAnimationFrame(() => this.beta37FinalCleanup());
    window.setTimeout(() => this.beta37FinalCleanup(), 90);
  };
