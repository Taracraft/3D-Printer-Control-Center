/* 3D-Printer Control Center - HACS Release 5.0.0-beta40*/
(() => {
  const VERSION = "5.0.0-beta40";
...
(() => {
  const STUDIO_VERSION = "5.0.0-beta40";
...
/* v5 beta40: Studio object/import/mouse recovery - continuation patch */
(() => {
  const FLAG = "__pccBeta40StudioObjectImportMouseFix";
  if (window[FLAG]) return;
  window[FLAG] = true;

  const ACTIVE_KEY = "printer_control_center_studio_beta40_active_job";
  const ACTIVE_TS_KEY = "printer_control_center_studio_beta40_active_ts";
  ...
  function recent() {
    try { return Date.now() - Number(localStorage.getItem(ACTIVE_TS_KEY) || 0) < RECENT_MS; }
    catch (_) { return false; }
  }

  function jobName(job) {
    return text(job?.name || job?.modelName || job?.model?.name || job?.file_name || job?.filename || job?.primitive?.label || job?.primitive_kind || "Studio-Objekt");
  }

  function jobId(job) {
    return text(job?.id || job?.job_id || job?.modelKey || job?.model?.id || jobName(job));
  }

  function jobPath(job) {
    return text(job?.file_path || job?.path || job?.model?.path || job?.plan?.file_path || job?.plan?.path || "");
  }

  function useful(job) {
    if (!job || typeof job !== "object") return false;
    const name = jobName(job);
    if (!name || name === "[object Object]" || name === "Studio Preview") return false;
    return true;
  }
  ...
  function compact(job) {
    ...
    const out = {};
    for (const k of ["id","job_id","name","modelName","file_name","filename","file_path","path","source","origin","primitive","primitive_kind","model","plan","transform","preview_data_url","preview_url","thumbnail","image"]) {
  ...
  function save(job) {
    if (!useful(job)) return;
    try {
      localStorage.setItem(ACTIVE_KEY, JSON.stringify(compact(job)));
      localStorage.setItem(ACTIVE_TS_KEY, String(Date.now()));
    } catch (_) {}
  }
  ...
  function plate(root) {
    return root?.querySelector?.(".buildplate") || root?.querySelector?.(".buildplate-wrap .buildplate") || null;
  }

  function onPlate(event, root) {
    const p = plate(root);
    if (!p) return null;
    const path = typeof event.composedPath === "function" ? event.composedPath() : [];
    if (path.includes(p)) return p;
    if (event.target?.closest?.(".buildplate")) return p;
    if (event.target?.closest?.(".studio-mesh-canvas")) return p;
    return null;
  }

  function installStyle(instance) {
    const root = instance?.shadowRoot;
    if (!root || root.querySelector("#pcc-beta40-studio-style")) return;

    const style = document.createElement("style");
    style.id = "pcc-beta40-studio-style";
    style.textContent = `
      .pcc-beta39-object-layer,
      .pcc-beta39-object,
      [data-pcc-beta39-object-id],
      .pcc-beta36-imported-image,
      .pcc-beta36-imported-placeholder {
        display:none!important;
        opacity:0!important;
      ...
      .pcc-beta30-color,
      .pcc-beta30-swatch,
      .pcc-beta30-menu-body:has(.pcc-beta30-color-row),
      .pcc-beta30-menu-body:has(.pcc-beta30-color),
      .pcc-beta30-menu-body:has(.pcc-beta30-swatch) {
        display:none!important;
        opacity:0!important;
        pointer-events:none!important;
      }
      ...
      .beta7-import-backdrop > * {
        ...
        overflow:hidden!important;
        border:1px solid rgba(0,190,255,.62)!important;
        border-radius:12px!important;
        background:linear-gradient(180deg,rgba(12,26,32,.98),rgba(4,13,18,.99))!important;
      ...
      .beta7-import-item,
      article[data-beta7-path] {
        min-height:136px!important;
        max-height:176px!important;
        padding:8px!important;
        border-radius:10px!important;
        border:1px solid rgba(0,190,255,.36)!important;
        background:rgba(7,20,26,.96)!important;
        overflow:hidden!important;
      }

      .beta7-import-backdrop button[data-action*="delete"],
      .beta7-import-backdrop button[data-action*="move"],
      .beta7-import-backdrop button[data-action*="rename"],
      .beta7-import-backdrop button[data-beta7-action="delete"],
      .beta7-import-backdrop button[data-beta7-action="move"],
      .beta7-import-backdrop button[data-beta7-action="rename"] {
        display:none!important;
      }
    `;
    root.appendChild(style);
  ...
  function sanitize(instance) {
    ...
    root.querySelectorAll(".pcc-beta39-object-layer,.pcc-beta39-object,[data-pcc-beta39-object-id],.pcc-beta36-imported-image,.pcc-beta36-imported-placeholder").forEach((n) => {
      try { n.remove(); } catch (_) { n.style.display = "none"; }
    });

    root.querySelectorAll(".pcc-beta30-color-row,.pcc-beta30-color,.pcc-beta30-swatch").forEach((n) => {
      n.style.display = "none";
      n.setAttribute("aria-hidden", "true");
    });

    const label = root.querySelector(".model-label");
    if (label && text(label.textContent).includes("[object Object]")) {
      label.textContent = "";
    }
    ...
    if (document.createTreeWalker) {
      ...
      while (walker.nextNode()) {
        if (String(walker.currentNode.nodeValue || "").includes("[object Object]")) {
          bad.push(walker.currentNode);
        }
      }
      bad.forEach((n) => {
        const clean = String(n.nodeValue || "").replaceAll("[object Object]", "").trim();
  ...
  function drawProxy(instance) {
    const root = instance?.shadowRoot;
    const canvas = root?.querySelector?.(".studio-mesh-canvas");
    if (!canvas) return false;

    const job = instance?._activeJob || load();
    if (!recent() || !useful(job)) return false;
    if (instance?._studioMesh?.triangles?.length) return false;

    const rect = canvas.getBoundingClientRect();
    const w = Math.max(320, Math.round(rect.width || canvas.clientWidth || 800));
    const h = Math.max(220, Math.round(rect.height || canvas.clientHeight || 520));
    const dpr = Math.max(1, Math.min(2, window.devicePixelRatio || 1));
    ...
    const t = instance._transform || {};
    const x = w / 2 + clamp(t.x || 0, -220, 220) * 2.2;
    const y = h / 2 + clamp(t.y || 0, -180, 180) * 2.2;
    const s = clamp(Number(t.scale || 100) / 100, 0.25, 3);
    const rz = Number(t.rz || 0) * Math.PI / 180;
    const name = jobName(job);
    ...
    ctx.fillStyle = "rgba(0, 172, 220, 0.92)";
    ctx.strokeStyle = "rgba(190, 245, 255, 0.82)";
    ctx.lineWidth = 1.4;
    ...
    ctx.beginPath();
    ctx.rect(-76, -46, 152, 92);
    ctx.fillStyle = "rgba(0, 138, 188, 0.92)";
    ctx.fill();
    ctx.stroke();
    ...
    ctx.lineTo(76, 46);
    ctx.closePath();
    ctx.fillStyle = "rgba(0, 102, 150, 0.90)";
    ctx.fill();
    ctx.stroke();
    ...
    ctx.strokeStyle = "rgba(0,190,255,.56)";
    ctx.beginPath();
    ctx.roundRect(bx, by, bw, 24, 8);
    ctx.fill();
    ctx.stroke();
    ...
    const status = root.querySelector(".mesh-status");
    if (status) status.textContent = `Sichtbarer Studio-Proxy aktiv: ${name}.`;

    return true;
  }

  function bindRightDrag(instance) {
    const root = instance?.shadowRoot;
    if (!root || root.dataset.pccBeta40RightDrag === "1") return;
    root.dataset.pccBeta40RightDrag = "1";
    ...
    const finish = (event) => {
      ...
      event?.stopPropagation?.();
      drag = null;
      try { instance.scheduleActiveJobSave?.(); } catch (_) {}
    ...
    const move = (event) => {
      ...
      instance._transform = { ...(instance._transform || {}) };
      instance._transform.x = Math.round(drag.originX + dx / 2.2);
      instance._transform.y = Math.round(drag.originY + dy / 2.2);

      if (instance._activeJob) {
        instance._activeJob.transform = { ...(instance._activeJob.transform || {}), ...instance._transform };
        save(instance._activeJob);
      }

      try { instance.renderMeshCanvas?.(); } catch (_) {}
      drawProxy(instance);
    };

    root.addEventListener("contextmenu", (event) => {
      if (onPlate(event, root)) {
        event.preventDefault();
        event.stopPropagation();
      }
    }, true);
    ...
    root.addEventListener("pointerdown", (event) => {
      const p = onPlate(event, root);
      if (!p) return;

      const right = event.button === 2 || Boolean((event.buttons || 0) & 2);
      if (!right) return;

      event.preventDefault();
      event.stopPropagation();

      if (useful(instance._activeJob)) save(instance._activeJob);

      drag = {
        startX: event.clientX,
        startY: event.clientY,
        originX: Number(instance._transform?.x || 0),
        originY: Number(instance._transform?.y || 0),
      };
  ...
  function cleanup(instance) {
    installStyle(instance);
    bindRightDrag(instance);
    sanitize(instance);
    drawProxy(instance);
  }
  ...
  function patch(ctor) {
    ...
    if (!proto || proto.__pccBeta40Patched) return;
    proto.__pccBeta40Patched = true;

    const oldRender = proto.render;
    proto.render = function beta40Render(...args) {
      const result = typeof oldRender === "function" ? oldRender.apply(this, args) : undefined;
      setTimeout(() => cleanup(this), 0);
      setTimeout(() => cleanup(this), 100);
      return result;
    };

    const oldCleanup = proto.cleanupBetaStudioUi;
    proto.cleanupBetaStudioUi = function beta40Cleanup(...args) {
      if (typeof oldCleanup === "function") {
        try { oldCleanup.apply(this, args); } catch (_) {}
      }
      cleanup(this);
    };

    const oldApply = proto.applyActiveJob;
    proto.applyActiveJob = function beta40ApplyActiveJob(job, options = {}) {
      const result = typeof oldApply === "function" ? oldApply.call(this, job, options) : undefined;
      if (useful(job)) {
        this._activeJob = { ...job };
        this._activeJobId = jobId(job);
        save(this._activeJob);
      }
      setTimeout(() => cleanup(this), 0);
      setTimeout(() => this.renderMeshCanvas?.(), 60);
      return result;
    };

    const oldLoad = proto.loadStudioJobs;
    proto.loadStudioJobs = async function beta40LoadStudioJobs(...args) {
      const stored = load();
      const result = typeof oldLoad === "function" ? await oldLoad.apply(this, args) : undefined;

      if (stored && Array.isArray(this._jobs)) {
        const found = this._jobs.find((j) => jobId(j) === jobId(stored) || jobPath(j) === jobPath(stored)) || stored;
        if (useful(found)) {
          this._activeJob = { ...found };
          this._activeJobId = jobId(found);
        }
      }
    ...
    const oldImportPlan = proto.beta7ImportPlan;
    proto.beta7ImportPlan = async function beta40ImportPlan(plan, ...args) {
      const result = typeof oldImportPlan === "function" ? await oldImportPlan.call(this, plan, ...args) : undefined;
      const job = this._activeJob || result?.job || result || plan;
      if (useful(job)) {
        this._activeJob = { ...job };
        this._activeJobId = jobId(job);
        save(this._activeJob);
      ...
      setTimeout(() => cleanup(this), 0);
      setTimeout(() => cleanup(this), 150);
      setTimeout(() => this.renderMeshCanvas?.(), 220);
      return result;
    };

    const oldImportSelected = proto.beta7ImportSelected;
    proto.beta7ImportSelected = async function beta40ImportSelected(...args) {
      const result = typeof oldImportSelected === "function" ? await oldImportSelected.apply(this, args) : undefined;
      if (useful(this._activeJob)) save(this._activeJob);
      setTimeout(() => cleanup(this), 0);
      setTimeout(() => cleanup(this), 150);
      return result;
    };

    const oldMesh = proto.renderMeshCanvas;
    proto.renderMeshCanvas = function beta40RenderMeshCanvas(...args) {
      const result = typeof oldMesh === "function" ? oldMesh.apply(this, args) : undefined;
      drawProxy(this);
      sanitize(this);
      return result;
    };
  ...
  const start = () => patch(customElements.get("printer-control-center-studio-card"));
  if (customElements.get("printer-control-center-studio-card")) start();
  else customElements.whenDefined("printer-control-center-studio-card").then(start).catch(() => {});
})();
